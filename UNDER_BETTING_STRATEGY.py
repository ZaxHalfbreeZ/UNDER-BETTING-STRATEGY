...
# 🌟 ฟังก์ชันสร้างไฟล์ Excel แยกตัว (ไม่ให้มัน Cache)
def create_excel_with_formula(df_edited, current_bankroll):
    output = io.BytesIO()
    try:
        # ตรวจสอบว่ามี xlsxwriter หรือไม่
        import xlsxwriter
    except ImportError:
        return None
...
if st.session_state.approved_matches:
    ...
    st.caption("💡 *ถ้าช่อง Odds เป็น 0.00 ให้แตะที่ตัวเลขแล้วพิมพ์ราคาน้ำจากเว็บพนันลงไป*")
    ...
    if final_bets:
        ...
        # 🌟 เรียกฟังก์ชันสร้าง Excel แบบไม่ใช้ Cache
        excel_file = create_excel_with_formula(edited_df, bankroll)
        
        if excel_file:
            st.download_button(
                label="📥 ดาวน์โหลดไฟล์ Excel (มีสูตรคำนวณให้!)",
                data=excel_file,
                file_name=f'Under_Bet_{datetime.now().strftime("%Y-%m-%d")}.xlsx',
                mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                use_container_width=True
            )
            st.caption("📌 *เปิดไฟล์ใน Excel แล้วลองแก้ตัวเลขในช่อง 'ใส่ Odds ตรงนี้' เงินแทงจะคำนวณให้อัตโนมัติ*")
        else:
            st.error("❌ ไม่สามารถสร้างไฟล์ Excel ได้ กรุณาตรวจสอบว่าไฟล์ requirements.txt มีคำว่า xlsxwriter แล้วหรือยัง?")
            
    else:
        st.warning("กรุณาใส่เลข Odds ที่มากกว่า 1.50 ในตารางด้านบน")
