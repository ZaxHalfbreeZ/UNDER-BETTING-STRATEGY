...
# ==========================================
# ปุ่มสแกน (จะทำงานเฉพาะตอนกดเท่านั้น)
# ==========================================
    ...
    if not games:
        ...
        for index, g in enumerate(games):
            ...
            try:
                ...
                score = calculate_score(combined_xg, poisson_under, xg_target) # มี typo ตรงนี้ แก้เป็น poisson_prob ด้านล่าง
        ...
        # 💡 สำคัญ: เก็บข้อมูลลงความจำ (Session State) ทันทีที่สแกนเสร็จ
        st.session_state.approved_matches = temp_approved
        st.session_state.near_misses = temp_near

# ==========================================
# ส่วนแสดงผล (อยู่นอกปุ่ม จะไม่หายถ้ามีการ Rerun)
# ==========================================
if st.session_state.approved_matches:
    ...
    # ==========================================
    # คำนวณเงินแทงจาก Odds ที่แก้ไขแล้ว (จะทำงานทันทีที่คุณพิมพ์ตัวเลข)
    # ==========================================
...
# ==========================================
# แสดง Near Misses (อยู่นอกปุ่มเช่นกัน)
# ==========================================
...
if not st.session_state.approved_matches and not st.session_state.near_misses:
    # จะแสดงก็ต่อเมื่อเพิ่งเข้ามาครั้งแรก หรือสแกนไปแล้วไม่เจออะไรเลย
    if st.button("🔍 เริ่มค้นหาคู่เกมวันนี้", key="empty_state_btn", type="secondary"):
        pass # ถ้ากดปุ่มนี้ มันจะ Rerun แต่จะไม่ทำอะไร เพราะปุ่มด้านบนจะจัดการ
