            ...
            if '2.5' in market_name or 'under/over' in market_name:
                for odd in market.get('odds', []):
                    if 'under' in odd.get('name', '').lower():
                        ...
                        if val and val >= 1.50:
                            return val
        return 0.0 # เปลี่ยนจาก None เป็น 0.0 เพื่อให้ใส่ในตารางแก้ไขได้
    except:
        return 0.0
...
def format_match_time(date_str):
    """แปลงเวลาจาก API ให้เป็นเวลาไทย (UTC+7)"""
    if not date_str:
        return "N/A"
    try:
        # แปลง string เป็น datetime object
        dt = datetime.fromisoformat(date_str)
        # ตั้งค่า Timezone ให้เป็น UTC ก่อน
        dt = dt.replace(tzinfo=timezone.utc)
        # แปลงเป็นเวลาไทย (บวก 7 ชม.)
...
if st.button("🔍 เริ่มค้นหาคู่เกมวันนี้", type="primary", use_container_width=True):
    ...
    if not games:
        ...
        for index, g in enumerate(games):
            ...
            raw_date = g.get('date') # ดึงเวลามาด้วย
            ...
            try:
                ...
                # จัดเรียงข้อมูลใหม่ พร้อมเพิ่มคอลัมน์เวลา
                match_data = {
                    ...
                    'คะแนน': score,
                    '✏️ Odds U2.5': u25_odds # เปลี่ยนชื่อเล็กน้อยเพื่อบอกว่าแก้ได้
                }
        ...
        # ==========================================
        # แสดงผลลัพธ์หลัก (ใช้ Data Editor แทน Dataframe)
        # ==========================================
        if approved_matches:
            st.divider()
            st.success(f"🎯 พบ **{len(approved_matches)} คู่** ที่ผ่านเกณฑ์!")
            ...
            # ใช้ st.data_editor เพื่อให้แก้ไขได้ (แต่ล็อคห้ามแก้คอลัมน์อื่น)
            ...
            # ==========================================
            # คำนวณเงินแทงจาก Odds ที่แก้ไขแล้ว
            # ==========================================
            ...
            for index, row in edited_df.iterrows():
                ...
                if odds >= 1.50: # ถ้ามีราคา หรือแก้ไขราคาแล้ว
