...
def format_match_time(date_str):
    ...
    try:
        ...
        thai_tz = timezone(timedelta(hours=4)) # แก้ไขเวลาให้ตรงประเทศไทยแล้ว
...
if st.session_state.approved_matches:
    ...
    if final_bets:
        ...
        # ==========================================
        # 🌟 สร้างไฟล์ Excel พร้อมสูตรคำนวณ
        # ==========================================
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            # ดึงเฉพาะแถวที่มี Odds สำหรับใส่ Excel
            excel_data = []
            for index, row in edited_df.iterrows():
                if row['✏️ Odds U2.5'] >= 1.50:
                    excel_data.append({
                        'คู่บอล': f"{row['ทีมเหย้า']} vs {row['ทีมเยือน']}",
                        '✏️ Odds U2.5 (ใส่ราคาตรงนี้)': row['✏️ Odds U2.5'],
                        'Poisson U2.5 (%)': row['Poisson U2.5 (%)']
                    })
            
            df_excel = pd.DataFrame(excel_data)
            
            # เขียนข้อมูลลง Excel (เริ่มจากแถวที่ 2 เพื่อเว้นที่ใส่หัวข้อ)
            df_excel.to_excel(writer, index=False, sheet_name='Bet Plan', startrow=1, header=False)
            
            workbook = writer.book
            worksheet = writer.sheets['Bet Plan']
            
            # จัดรูปแบบหัวข้อ
            header_format = workbook.add_format({'bold': True, 'align': 'center', 'bg_color': '#4F81BD', 'font_color': 'white', 'border': 1})
            headers = ['คู่บอล', '✏️ Odds U2.5 (ใส่ราคาตรงนี้)', 'Poisson U2.5 (%)', '💰 แนะนำแทง (บาท)']
            ...
            # 🌟 ฝังสูตร Kelly ลงในคอลัมน์ D (เงินแทง)
            # B = Odds, C = Poisson
            for row_num in range(1, len(excel_data) + 1):
                # สูตร Excel: =IF(B>=1.5, MIN(MAX((((B-1)*(C/100)-(1-(C/100)))/(B-1))*30, 0), 5) * เงินทุน / 100, 0)
                formula = f'=IF(B{row_num}>=1.5, MIN(MAX((((B{row_num}-1)*(C{row_num}/100)-(1-(C{row_num}/100)))/(B{row_num}-1))*30, 0), 5) * {bankroll} / 100, 0)'
                worksheet.write_formula(row_num, 3, formula)
                
            # จัดรูปแบบเงินให้มีเครื่องหมาย ฿
        ...
        st.download_button(
            label="📥 ดาวน์โหลดเป็นไฟล์ Excel (มีสูตรคำนวณให้!)",
            data=output,
            file_name=f'Under_Bet_{datetime.now().strftime("%Y-%m-%d")}.xlsx',
            mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            use_container_width=True
        )
        st.caption("📌 *เปิดไฟล์ใน Excel หรือ Google Sheets แล้วลองแก้ตัวเลขในช่อง Odds ดูสิครับ เงินแทงจะคำนวณให้ทันทีเอง*")
